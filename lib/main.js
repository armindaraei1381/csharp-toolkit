...
/**
 * csharp-toolkit — main entry point.
 * (formerly published as `csharpextensions-pulsar`)
 *
 * A native Pulsar (Atom API) C#/.NET toolkit: project scaffolding via the
 * dotnet CLI, file templates, constructors and more.
 *
 * Author: Armin Daraei
 */
...
import CreateDotnetProjectCommand from './commands/create-dotnet-project';
import ProjectKindDetector from './core/project-kind-detector';
import i18n from './core/i18n';
import { PACKAGE_NAME, LEGACY_PACKAGE_NAME, commandName, configKey } from './core/package-meta';
...
export default {
  ...
  activate() {
    ...
    for (const [name, kind, options = {}] of NEW_FILE_COMMANDS) {
      ...
      this.subscriptions.add(
        atom.commands.add('atom-workspace', commandName(name), (event) => command.run(event))
      );
    }
    ...
    for (const kind of DUAL_FILE_COMMANDS) {
      ...
      this.subscriptions.add(
        atom.commands.add('atom-workspace', commandName(`new-${kind}`), (event) => command.run(event))
      );
    }
    ...
    const constructorCommand = new ConstructorFromPropertiesCommand({
      expressionBodied: () =>
        atom.config.get(configKey('constructorStyle')) === 'expression-bodied'
    });
    const expressionConstructorCommand = new ConstructorFromPropertiesCommand({
      expressionBodied: true
    });

    // --- .NET project creation (dotnet CLI) ---
    const createProjectCommand = new CreateDotnetProjectCommand();

    this.subscriptions.add(
      atom.commands.add('atom-workspace', {
        [commandName('constructor-from-properties')]: (event) => constructorCommand.run(event),
        [commandName('constructor-from-properties-expression-body')]: (event) =>
          expressionConstructorCommand.run(event),
        [commandName('create-dotnet-project')]: (event) => createProjectCommand.run(event),
        [commandName('reload-custom-templates')]: () => this.loadCustomTemplates(true)
      })
    );
  ...
  /**
   * Copies user settings from the pre-rename config scope
   * (`csharpextensions-pulsar.*`) into the new one (`csharp-toolkit.*`).
   * Non-destructive: legacy keys are left untouched. Runs once per session
   * and is effectively a no-op after the first write.
   */
  migrateLegacySettings() {
    try {
      const legacy = atom.config.get(LEGACY_PACKAGE_NAME);
      if (!legacy || typeof legacy !== 'object') return;
      const currentRaw = (atom.config.settings && atom.config.settings[PACKAGE_NAME]) || {};
      for (const [key, value] of Object.entries(legacy)) {
        if (!(key in currentRaw) && value !== null && typeof value !== 'object') {
          atom.config.set(`${PACKAGE_NAME}.${key}`, value);
        }
      }
    } catch (error) {
      // Migration is best-effort — never block activation.
      console.error('[csharp-toolkit] settings migration failed:', error);
    }
  },
  ...
  /**
   * Grouped "New C#" context menu for the tree view. Registered from JS
   * (not menus/*.json) because conditional item visibility (`shouldDisplay`)
   * — required by the ASP.NET / UWP auto-hide settings — only exists in the
   * context-menu API; static JSON cannot express it.
   */
  registerTreeViewContextMenu() {
    ...
    const commandItem = (label, name, shouldDisplay = null) => {
      const item = { label, command: commandName(name) };
  ...
  shouldShowContextGroup(event, kind) {
    ...
    if (!atom.config.get(configKey(setting))) return true;
  ...
  loadCustomTemplates(announce) {
    ...
    for (const entry of entries.sort()) {
      ...
      this.customTemplateSubscriptions.add(
        atom.commands.add('atom-workspace', commandName(`custom-${slug}`), (event) =>
          command.run(event)
        )
      );
      menuItems.push({ label: id, command: commandName(`custom-${slug}`) });
    }
    ...
    if (menuItems.length > 0) {
      ...
      const mainMenu = atom.menu.add([
        {
          label: 'Packages',
          submenu: [{ label: 'CSharp Toolkit', submenu: [{ label: menuLabel, submenu: menuItems }] }]
        }
      ]);
