...
export default class ConstructorFromPropertiesCommand {
  ...
  get config() {
    return {
      confirmUsingDirectives: atom.config.get('csharpextensions-pulsar.confirmUsingDirectives')
    };
  }
  ...
  /** Finds `class|struct|interface|record [struct|class] Name {` declarations. */
  ...
  /** Extracts non-static auto-properties with a `set` or `init` accessor. */
  ...
  /** `Name` → `name`, `ID` → `id`, reserved words → verbatim `@word`. */
  ...
  /** Using-Directive Auto-Suggest (section 2, item 4 of the feature list). */
